...
class FrameworkStepController extends FormWizardController {
  ...
  checkEditable(req, res, next) {
    const isEditable = req.youthRiskAssessment?.isEditable
    const canEdit = true
  ...
  setValidationRules(req, res, next) {
    const { responses } = req.youthRiskAssessment || {}
  ...
  setInitialValues(req, res, next) {
    ...
    const responses = req.youthRiskAssessment.responses
  ...
  setSyncStatusBanner(req, res, next) {
    const { nomis_sync_status: syncStatus } = req.youthRiskAssessment
  ...
  setPrefillBanner(req, res, next) {
    const { prefill_source: source, responses } = req.youthRiskAssessment || {}
    const fields = req.form.options.fields
  ...
  async saveValues(req, res, next) {
    const { form, youthRiskAssessment } = req
    const responses = youthRiskAssessment.responses
      .filter(response =>
    ...
    try {
      ...
      if (responses.length) {
        await youthRiskAssessmentService.respond(
          youthRiskAssessment.id,
          responses
        )
      }
